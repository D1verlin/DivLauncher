...
export default function ServerPage({ logs, isRunning, players, currentPack, onPackUpdate }) {
  ...
  const renderColorizedLogs = (logsText) => {
    if (!logsText) return <span style={{ color: '#52525b' }}>{lang === 'ru' ? 'Ожидание запуска ядра...' : 'Waiting for core launch...'}</span>;
  ...
  const loadBackups = async () => {
    ...
    try {
      ...
      console.error(lang === 'ru' ? 'Ошибка загрузки бэкапов:' : 'Error loading backups:', e);
    } finally {
  ...
  const handleDeleteBackup = async (fileName) => {
    if (!currentPack) return;
    if (confirm(lang === 'ru' ? `Вы уверены, что хотите удалить бэкап ${fileName}?` : `Are you sure you want to delete backup ${fileName}?`)) {
      const res = await window.electronAPI.deleteBackup(currentPack, fileName);
      if (res.success) {
        loadBackups();
      } else {
        alert((lang === 'ru' ? 'Ошибка удаления: ' : 'Delete error: ') + res.error);
      }
    }
  };
  ...
  const handleRestoreBackup = async (fileName) => {
    if (!currentPack) return;
    if (isRunning) {
      alert(lang === 'ru' ? 'Невозможно восстановить бэкап при запущеном сервере! Сначала остановите сервер.' : 'Cannot restore backup while the server is running! Stop the server first.');
      return;
    }
    if (confirm(lang === 'ru' ? `Вы действительно хотите ВОССТАНОВИТЬ мир из ${fileName}? Текущий мир будет полностью удален.` : `Are you sure you want to RESTORE the world from ${fileName}? The current world will be completely deleted.`)) {
      setIsRestoring(true);
      const res = await window.electronAPI.restoreBackup(currentPack, fileName);
      setIsRestoring(false);
      if (res.success) {
        alert(lang === 'ru' ? 'Резервная копия успешно восстановлена!' : 'Backup restored successfully!');
        loadBackups();
      } else {
        alert((lang === 'ru' ? 'Ошибка восстановления: ' : 'Restore error: ') + res.error);
      }
    }
  };
  ...
  const handleSyncMods = async () => {
    if (window.electronAPI && window.electronAPI.syncLocalMods) {
      ...
      if (result.success) {
        alert(lang === 'ru' ? `Успешно подтянуто модов: ${result.copiedCount} (клиентских пропущено: ${result.skippedCount})` : `Successfully synced mods: ${result.copiedCount} (skipped client-only: ${result.skippedCount})`);
      } else {
        alert((lang === 'ru' ? 'Ошибка синхронизации: ' : 'Sync error: ') + result.error);
      }
    }
  };
  ...
  const saveProps = async () => {
    ...
    setHasUnsavedChanges(false);
    if (isRunning) window.electronAPI.sendServerCommand(lang === 'ru' ? 'say Настройки изменены (требуется рестарт)' : 'say Settings changed (restart required)');
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', background: 'rgba(0,0,0,0.2)', padding: '10px', borderRadius: '16px' }}>
        <h2 style={{ margin: '0 20px 0 10px', fontSize: '18px', fontWeight: 700, color: '#fff' }}>{lang === 'ru' ? 'Сервер' : 'Server'}</h2>
        <TabButton active={activeTab === 'console'} icon="fa-terminal" label={lang === 'ru' ? 'Консоль' : 'Console'} onClick={() => setActiveTab('console')} />
        <TabButton active={activeTab === 'players'} icon="fa-users" label={(lang === 'ru' ? 'Игроки' : 'Players') + ` (${players.length})`} onClick={() => setActiveTab('players')} />
        <TabButton active={activeTab === 'backups'} icon="fa-file-zipper" label={lang === 'ru' ? 'Бэкапы' : 'Backups'} onClick={() => setActiveTab('backups')} />
        <TabButton active={activeTab === 'settings'} icon="fa-sliders" label={lang === 'ru' ? 'Настройки' : 'Settings'} onClick={() => setActiveTab('settings')} />
      </div>
          ...
          <motion.div variants={itemVariants} initial="hidden" animate="visible" style={{ display: 'flex', gap: '15px', height: '100%' }}>
            ...
            <div style={{ width: '220px', minWidth: '220px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ ...glassPanelStyle, padding: '15px' }}>
                <div style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 600, marginBottom: '6px' }}>{lang === 'ru' ? 'Статус' : 'Status'}</div>
                <div style={{ fontSize: '15px', fontWeight: 700, color: isRunning ? '#10b981' : '#ef4444', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {isRunning ? (lang === 'ru' ? 'Работает' : 'Running') : (lang === 'ru' ? 'Отключен' : 'Offline')}
                </div>
              </div>
              ...
              >
                {isRunning ? <><i className="fa-solid fa-power-off"></i> {lang === 'ru' ? 'Выключить' : 'Stop'}</> : <><i className="fa-solid fa-play"></i> {lang === 'ru' ? 'Запустить' : 'Start'}</>}
              </motion.button>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                ...
                <motion.button whileHover={isRunning ? { background: 'rgba(16, 185, 129, 0.2)' } : {}} onClick={() => window.electronAPI.sendServerCommand('save-all')} disabled={!isRunning} style={{ ...glassPanelStyle, borderRadius: '12px', padding: '10px', cursor: isRunning ? 'pointer' : 'default', fontSize: '12px', fontWeight: 600, color: isRunning ? '#34d399' : '#52525b', justifyContent: 'center', alignItems: 'center', gap: '6px', transition: 'background 0.2s' }}>
                  <i className="fa-solid fa-floppy-disk"></i> {lang === 'ru' ? 'Сохранить' : 'Save'}
                </motion.button>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <motion.button whileHover={{ background: 'rgba(255,255,255,0.1)' }} onClick={() => window.electronAPI.backupWorld(currentPack)} style={{ ...glassPanelStyle, borderRadius: '12px', padding: '10px 15px', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', fontSize: '12px', fontWeight: 600, color: '#f59e0b', transition: 'background 0.2s' }}>
                    <i className="fa-solid fa-box-archive"></i> {lang === 'ru' ? 'Бэкап мира' : 'World Backup'}
                  </motion.button>
                  <motion.button whileHover={{ background: 'rgba(255,255,255,0.1)' }} onClick={() => window.electronAPI.openServerFolder(currentPack)} style={{ ...glassPanelStyle, borderRadius: '12px', padding: '10px 15px', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', fontSize: '12px', fontWeight: 600, color: '#e4e4e7', transition: 'background 0.2s' }}>
                    <i className="fa-regular fa-folder-open"></i> {lang === 'ru' ? 'Папка сервера' : 'Server Folder'}
                  </motion.button>
                  ...
                  {logs && logs.trim() !== '' && (
                    <motion.button whileHover={{ background: 'rgba(255,255,255,0.1)' }} onClick={handleExportServerLogs} style={{ ...glassPanelStyle, borderRadius: '12px', padding: '10px 15px', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', fontSize: '12px', fontWeight: 600, color: '#e4e4e7', transition: 'background 0.2s' }}>
                      <i className="fa-solid fa-file-arrow-down"></i> {lang === 'ru' ? 'Экспорт логов' : 'Export Logs'}
                    </motion.button>
                  )}
                  ...
                  {currentPack.isCustom && (
                    ...
                    >
                      <i className="fa-solid fa-arrows-rotate"></i> {lang === 'ru' ? 'Подтянуть моды' : 'Sync Mods'}
                    </motion.button>
                  )}
              </div>
            </div>
              ...
              <div style={{ padding: '12px' }}>
                <form onSubmit={sendCommand} style={{ display: 'flex', gap: '10px', alignItems: 'center', background: 'rgba(0, 0, 0, 0.4)', border: '1px solid rgba(255,255,255,0.05)', padding: '6px 12px', borderRadius: '12px', transition: 'border 0.2s' }}>
                  ...
                  <input 
                    ...
                    placeholder={isRunning ? (lang === 'ru' ? 'Введите команду...' : 'Enter command...') : (lang === 'ru' ? 'Сервер выключен' : 'Server is offline')} 
            ...
            <div style={{ ...glassPanelStyle, padding: '20px', flexGrow: 1, overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h3 style={{ margin: 0, color: '#e4e4e7', fontSize: '15px', fontWeight: 600 }}>
                  {lang === 'ru' ? 'Онлайн' : 'Online'} <span style={{ color: '#10b981', background: 'rgba(16,185,129,0.1)', padding: '2px 8px', borderRadius: '8px', marginLeft: '8px' }}>{players.length}</span>
                </h3>
                {isRunning && (
                  <form onSubmit={(e) => { e.preventDefault(); const v = e.target.elements.cmd.value; if(v) window.electronAPI.sendServerCommand(`whitelist add ${v}`); e.target.reset(); }} style={{ display: 'flex', gap: '8px' }}>
                    <input name="cmd" type="text" placeholder={lang === 'ru' ? 'Ник игрока...' : 'Player nickname...'} style={{ ...glassInputStyle, width: '200px' }} />
                    <button type="submit" style={{ background: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.3)', color: '#60a5fa', padding: '0 15px', borderRadius: '12px', cursor: 'pointer', fontWeight: 600, fontSize: '12px' }}>{lang === 'ru' ? 'Добавить в Whitelist' : 'Add to Whitelist'}</button>
                  </form>
                )}
              </div>
              ...
              {players.length === 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#52525b', gap: '15px' }}>
                  ...
                  <div style={{ fontSize: '14px', fontWeight: 500 }}>{lang === 'ru' ? 'Сервер пуст' : 'Server is empty'}</div>
                </div>
              ) : (
                  ...
                  {players.map(player => (
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <motion.button whileHover={{ scale: 1.1, color: '#10b981' }} onClick={() => handlePlayerAction('op', player)} title={lang === 'ru' ? 'Дать OP' : 'Give OP'} style={{ background: 'transparent', border: 'none', color: '#71717a', cursor: 'pointer', fontSize: '13px' }}>
                          <i className="fa-solid fa-crown" />
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.1, color: '#6366f1' }} onClick={() => handlePlayerAction('deop', player)} title={lang === 'ru' ? 'Забрать OP' : 'Revoke OP'} style={{ background: 'transparent', border: 'none', color: '#71717a', cursor: 'pointer', fontSize: '13px' }}>
                          <i className="fa-solid fa-user-shield" />
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.1, color: '#f59e0b' }} onClick={() => handlePlayerAction('kick', player)} title={lang === 'ru' ? 'Кикнуть' : 'Kick'} style={{ background: 'transparent', border: 'none', color: '#71717a', cursor: 'pointer', fontSize: '13px' }}>
                          <i className="fa-solid fa-user-minus" />
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.1, color: '#ef4444' }} onClick={() => handlePlayerAction('ban', player)} title={lang === 'ru' ? 'Забанить' : 'Ban'} style={{ background: 'transparent', border: 'none', color: '#71717a', cursor: 'pointer', fontSize: '13px' }}>
          ...
          <motion.div variants={itemVariants} initial="hidden" animate="visible" style={{ ...glassPanelStyle, height: '100%', position: 'relative' }}>
            <div style={{ padding: '20px', flexGrow: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '20px', paddingBottom: '100px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
                ...
                <div style={{ background: 'rgba(0,0,0,0.15)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.03)' }}>
                  <h4 style={{ color: '#e4e4e7', marginTop: 0, marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '14px', fontWeight: 600 }}><i className="fa-solid fa-globe" style={{color:'#3b82f6'}}></i> {lang === 'ru' ? 'Основные' : 'General'}</h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div>
                      <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500, marginBottom: '6px', display: 'block' }}>{lang === 'ru' ? 'Описание (MOTD)' : 'Description (MOTD)'}</label>
                      <input type="text" value={propsData['motd'] || ''} onChange={e => handlePropChange('motd', e.target.value)} style={glassInputStyle} />
                    </div>
                    {currentPack.isCustom && (
                      <div>
                        <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500, marginBottom: '6px', display: 'block' }}>{lang === 'ru' ? 'Ядро сервера' : 'Server Core'}</label>
                        <select 
                          value={serverLoader} 
                        ...
                        {getServerCoreOptions().length <= 1 && (
                          <span style={{ fontSize: '10px', color: '#f59e0b', marginTop: '4px', display: 'block' }}>
                            {lang === 'ru' ? 'Для данного клиента доступно только одно совместимое ядро.' : 'Only one compatible core is available for this client.'}
                          </span>
                        )}
                      </div>
                    )}
                    <div style={{ display: 'flex', gap: '12px' }}>
                      <div style={{ flexGrow: 1 }}>
                        <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500, marginBottom: '6px', display: 'block' }}>{lang === 'ru' ? 'Слоты' : 'Slots'}</label>
                      ...
                      <div style={{ flexGrow: 1 }}>
                        <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500, marginBottom: '6px', display: 'block' }}>{lang === 'ru' ? 'Порт сервера' : 'Server Port'}</label>
                ...
                {/* Gameplay Settings */}
                <div style={{ background: 'rgba(0,0,0,0.15)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.03)' }}>
                  <h4 style={{ color: '#e4e4e7', marginTop: 0, marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '14px', fontWeight: 600 }}><i className="fa-solid fa-gamepad" style={{color:'#f59e0b'}}></i> {lang === 'ru' ? 'Игровой процесс' : 'Gameplay'}</h4>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div>
                      <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500, marginBottom: '6px', display: 'block' }}>{lang === 'ru' ? 'Сложность' : 'Difficulty'}</label>
                      <select value={propsData['difficulty'] || 'normal'} onChange={e => handlePropChange('difficulty', e.target.value)} style={{...glassInputStyle, cursor: 'pointer'}}>
                        <option value="peaceful" style={{background: '#111'}}>{lang === 'ru' ? 'Мирная' : 'Peaceful'}</option>
                        <option value="easy" style={{background: '#111'}}>{lang === 'ru' ? 'Лёгкая' : 'Easy'}</option>
                        <option value="normal" style={{background: '#111'}}>{lang === 'ru' ? 'Нормальная' : 'Normal'}</option>
                        <option value="hard" style={{background: '#111'}}>{lang === 'ru' ? 'Сложная' : 'Hard'}</option>
                      </select>
                    </div>
                    <div>
                      <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500, marginBottom: '6px', display: 'block' }}>{lang === 'ru' ? 'Режим игры' : 'Game Mode'}</label>
                      <select value={propsData['gamemode'] || 'survival'} onChange={e => handlePropChange('gamemode', e.target.value)} style={{...glassInputStyle, cursor: 'pointer'}}>
                        <option value="survival" style={{background: '#111'}}>{lang === 'ru' ? 'Выживание' : 'Survival'}</option>
                        <option value="creative" style={{background: '#111'}}>{lang === 'ru' ? 'Творчество' : 'Creative'}</option>
                        <option value="adventure" style={{background: '#111'}}>{lang === 'ru' ? 'Приключение' : 'Adventure'}</option>
                        <option value="spectator" style={{background: '#111'}}>{lang === 'ru' ? 'Наблюдатель' : 'Spectator'}</option>
                      </select>
                    </div>
                    <div>
                      ...
                      <select value={propsData['pvp'] || 'true'} onChange={e => handlePropChange('pvp', e.target.value)} style={{...glassInputStyle, cursor: 'pointer'}}>
                        <option value="true" style={{background: '#111'}}>{lang === 'ru' ? 'Вкл' : 'On'}</option>
                        <option value="false" style={{background: '#111'}}>{lang === 'ru' ? 'Выкл' : 'Off'}</option>
                      </select>
                    </div>
                    <div>
                ...
                {/* World & Security */}
                <div style={{ background: 'rgba(0,0,0,0.15)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.03)' }}>
                  <h4 style={{ color: '#e4e4e7', marginTop: 0, marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '14px', fontWeight: 600 }}><i className="fa-solid fa-shield-halved" style={{color:'#10b981'}}></i> {lang === 'ru' ? 'Безопасность' : 'Security'}</h4>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div>
                      <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500, marginBottom: '6px', display: 'block' }}>{lang === 'ru' ? 'Лицензия' : 'Online Mode'}</label>
                      <select value={propsData['online-mode'] || 'true'} onChange={e => handlePropChange('online-mode', e.target.value)} style={{...glassInputStyle, cursor: 'pointer'}}>
                        <option value="false" style={{background: '#111'}}>{lang === 'ru' ? 'Пиратки (false)' : 'Offline (false)'}</option>
                        <option value="true" style={{background: '#111'}}>{lang === 'ru' ? 'Лицензия (true)' : 'Online (true)'}</option>
                      </select>
                    </div>
                    <div>
                      <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500, marginBottom: '6px', display: 'block' }}>{lang === 'ru' ? 'Вайтлист' : 'Whitelist'}</label>
                    ...
                    <div style={{ gridColumn: '1 / -1' }}>
                      <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500, marginBottom: '6px', display: 'block' }}>{lang === 'ru' ? 'Радиус защиты спавна (в блоках)' : 'Spawn Protection Radius (blocks)'}</label>
                ...
                {/* Optimization */}
                <div style={{ background: 'rgba(0,0,0,0.15)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.03)' }}>
                  <h4 style={{ color: '#e4e4e7', marginTop: 0, marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '14px', fontWeight: 600 }}><i className="fa-solid fa-rocket" style={{color:'#ec4899'}}></i> {lang === 'ru' ? 'Оптимизация мира' : 'World Optimization'}</h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                        <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500 }}>{lang === 'ru' ? 'Дальность чанков' : 'View Distance'}</label>
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                        <label style={{ fontSize: '12px', color: '#a1a1aa', fontWeight: 500 }}>{lang === 'ru' ? 'Дистанция симуляции' : 'Simulation Distance'}</label>
            ...
            <AnimatePresence>
              {hasUnsavedChanges && (
                ...
                >
                  <span style={{ color: '#e4e4e7', fontWeight: 500, fontSize: '13px' }}>{lang === 'ru' ? 'У вас есть несохранённые изменения' : 'You have unsaved changes'}</span>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <motion.button 
          ...
          <motion.div variants={itemVariants} initial="hidden" animate="visible" style={{ ...glassPanelStyle, height: '100%' }}>
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', height: '100%', gap: '15px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <h3 style={{ margin: 0, color: '#fff', fontSize: '16px', fontWeight: 800 }}>{lang === 'ru' ? 'Резервные копии мира' : 'World Backups'}</h3>
                  <p style={{ margin: '4px 0 0 0', color: '#71717a', fontSize: '12px', fontWeight: 600 }}>
                    {lang === 'ru' ? 'Архивы папки world. Восстановление перезапишет текущий мир.' : 'Archives of world folder. Restoring overwrites current world.'}
                  </p>
                </div>
                <motion.button
                ...
                >
                  <i className="fa-solid fa-file-zipper"></i> {lang === 'ru' ? 'Создать бэкап' : 'Create Backup'}
                </motion.button>
              </div>
              ...
              {isRestoring && (
                <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', padding: '12px', borderRadius: '12px', color: '#f87171', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span>{lang === 'ru' ? 'Восстановление мира в процессе... Пожалуйста, подождите.' : 'World restoration in progress... Please wait.'}</span>
                </div>
              )}

              {backupsLoading ? (
                <div style={{ display: 'flex', flexGrow: 1, alignItems: 'center', justifyContent: 'center', color: '#a1a1aa' }}>
                  <i className="fa-solid fa-circle-notch fa-spin" style={{ fontSize: '24px', marginRight: '10px' }}></i> {lang === 'ru' ? 'Загрузка списка резервных копий...' : 'Loading backups list...'}
                </div>
              ) : backups.length === 0 ? (
                <div style={{ display: 'flex', flexGrow: 1, flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#52525b', gap: '15px' }}>
                  ...
                  <div style={{ fontSize: '14px', fontWeight: 500 }}>{lang === 'ru' ? 'Бэкапы не найдены' : 'No backups found'}</div>
                </div>
              ) : (
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        ...
                        <div>
                          ...
                          <div style={{ color: '#71717a', fontSize: '11px', fontWeight: 600, marginTop: '2px' }}>
                            {lang === 'ru' ? 'Размер: ' : 'Size: '}{(backup.size / (1024 * 1024)).toFixed(2)} {lang === 'ru' ? 'МБ' : 'MB'} • {lang === 'ru' ? 'Создан: ' : 'Created: '}{new Date(backup.time).toLocaleString()}
                          </div>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <motion.button
                          ...
                          title={isRunning ? (lang === 'ru' ? "Остановите сервер для восстановления" : "Stop server to restore") : (lang === 'ru' ? "Восстановить мир из этого бэкапа" : "Restore world from this backup")}
                        >
                          <i className="fa-solid fa-clock-rotate-left"></i> {lang === 'ru' ? 'Восстановить' : 'Restore'}
                        </motion.button>
                        <motion.button
                          ...
                          title={lang === 'ru' ? "Удалить этот бэкап" : "Delete this backup"}
                        >
                          <i className="fa-solid fa-trash-can"></i> {lang === 'ru' ? 'Удалить' : 'Delete'}
                        </motion.button>
                      </div>
                    </motion.div>
                  ))}
